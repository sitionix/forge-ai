...
class CompleteArchitectLaneOrchestrationUseCaseTest {
    ...
    @AfterEach
    void tearDown() {
        verifyNoMoreInteractions(this.agentTicketApiMapper, this.createAgentTask, this.servicePropertiesProvider, this.serviceConfigView);
    }

    @Test
    void givenBackendScopeAndApiRequiredAndEventNotRequired_whenComplete_thenCreateBeAndApiAndMarkEventNotNeeded() {
        //given
    ...
    @Test
    void givenFrontendScopeAndApiNotRequiredAndEventRequired_whenComplete_thenCreateFeAndEventAndMarkApiNotNeeded() {
        //given
        final UUID ticketId = UUID.randomUUID();
        final UUID laneId = UUID.randomUUID();
        final CompleteArchitectLaneRequest request = CompleteArchitectLaneRequest.builder()
