...
def prepare(session,rootfs):
    ...
    if manifest.exists():raise ValueError('Session context already prepared; refusing replacement')
    user='frw-'+uuid.UUID(session).hex[:24]
    try:pwd.getpwnam(user)
    except KeyError:pass
    else:raise ValueError('Workload identity already allocated')
    ...
    workspace=parent/session;workspace.mkdir(mode=0o700);os.chown(workspace,account.pw_uid,account.pw_gid)
