...
Implement backend-owned unit-test lane behavior for the assigned service scope using the lane task, runtime context, provided contracts, and generated artifacts.

The result of this lane is unit-test implementation plus a successful `test-unit` completion callback.
...
- Implement only the requested unit-test lane behavior.
...
After unit-test implementation is complete, call the provided `test-unit` completion endpoint.

Build the request from the provided OpenAPI completion contract reference and runtime values.

The completion payload represents unit-test implementation facts only.
