...
class KnowledgeFreshnessService:
    ...
    def check(self, source_ids: Optional[List[str]] = None, groups: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        stored_files = self.store.stored_files(selected_source_ids if selected_source_ids else None)
        current_files = self._current_files(selected_sources)
        result = self._compare(stored_files, current_files)
        by_source: Dict[str, Dict[str, Any]] = {}
        for source_id in sorted(set(selected_source_ids) | {row["sourceId"] for row in stored_files} | {file.sourceId for file in current_files}):
            source_stored = [row for row in stored_files if row["sourceId"] == source_id]
            source_current = [file for file in current_files if file.sourceId == source_id]
            by_source[source_id] = self._compare(source_stored, source_current)
        result["sources"] = by_source
        return result

    def check_stored_files(self, stored_files: List[Dict[str, Any]], source_ids: Optional[List[str]] = None) -> Dict[str, Any]:
        selected_sources = self._selected_sources(source_ids or [], [])
        current_files = self._current_files(selected_sources)
        return self._compare(stored_files, current_files)
    ...
    def _compare(self, stored_files: List[Dict[str, Any]], current_files: List[FileMetadata]) -> Dict[str, Any]:
        stored_by_key = {(row["sourceId"], row["relativePath"]): row for row in stored_files}
        ...
        deleted_keys = stored_keys - current_keys
        modified_keys = {key for key in stored_keys & current_keys if stored_by_key[key]["contentHash"] != current_by_key[key].contentHash}
        affected_ids = [stored_by_key[key]["id"] for key in deleted_keys | modified_keys if stored_by_key[key].get("id") is not None]
