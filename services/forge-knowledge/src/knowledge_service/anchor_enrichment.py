...
class AnchorAwareGraphValidator:
    def merge(self,
              ...
              enrichment: Optional[GraphAnalysisResult],
              line_count: int) -> GraphAnalysisResult:
        ...
        for edge in enrichment.edges:
            ...
            accepted_edges.append(edge.copy(update={
    ...
    def _resolve_claim_target(self,
                              claim: GraphClaim,
                              anchors: Dict[str, GraphNode],
                              source_nodes: Dict[str, GraphNode]) -> Optional[str]:
        direct = self._resolve_edge_endpoint(claim.nodeLocalId, anchors, source_nodes)
        if direct:
            return direct
        return None

    def _resolve_edge_endpoint(self,
        ...
        if source_node is None:
            return None
        candidates = [
            anchor for anchor in anchors.values()
            if anchor.nodeKind == source_node.nodeKind
            and source_node.qualifiedName
            and anchor.qualifiedName == source_node.qualifiedName
        ]
        if len(candidates) == 1:
        ...
        candidates = [
            anchor for anchor in anchors.values()
            if anchor.nodeKind == source_node.nodeKind
            and anchor.name == source_node.name
        ]
        if len(candidates) == 1:
            return candidates[0].localId
        return None

    def _validated_claim(self,
                         claim: GraphClaim,
                         target_local_id: str,
                         node: GraphNode,
                         line_count: int,
                         diagnostics: List[Dict[str, Any]]) -> GraphClaim:
