...
def load_source_config(path: Path) -> Optional[SourceConfig]:
    ...
    catalog_path_value = str(catalog.get("path") or "").strip()
    workspace_root_value = str(catalog.get("workspace_root") or "").strip()
    if not catalog_path_value:
        raise KnowledgeError("KNOWLEDGE_CONFIG_INVALID", "catalog.path is required")
    if not workspace_root_value:
        raise KnowledgeError("KNOWLEDGE_CONFIG_INVALID", "catalog.workspace_root is required")
    catalog_path = resolve_config_path(catalog_path_value, config_file=path)
    workspace_root = resolve_config_path(workspace_root_value, config_file=path)
