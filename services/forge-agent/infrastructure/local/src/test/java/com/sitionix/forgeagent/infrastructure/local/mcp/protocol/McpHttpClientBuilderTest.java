...
class McpHttpClientBuilderTest {
    @Test void sdkCannotReplaceTheNoProxyPolicy() {
        var builder = new McpHttpClientBuilder();
        assertThatThrownBy(() -> builder.proxy(ProxySelector.of(new InetSocketAddress("127.0.0.1", 9999))))
                .isInstanceOf(IllegalArgumentException.class);
    }

    @Test void authorizationStatusSurvivesBodyHandlerFailure() throws Exception {
        ...
        try {
            ...
            assertThatThrownBy(() -> client.send(request, info -> {
                throw new IllegalStateException("synthetic body handler failure");
            })).isNotNull();
            assertThat(builder.authStatus()).isEqualTo(401);
    ...
    @Test void laterPostDoesNotEraseObservedForbiddenStatusBeforeItCompletes() throws Exception {
        ...
        try {
            ...
            assertThatThrownBy(() -> client.send(request, HttpResponse.BodyHandlers.discarding()))
                    .isInstanceOf(McpHttpClientBuilder.AuthStatus.class);
            ...
            assertThatThrownBy(pending::join).hasCauseInstanceOf(McpHttpClientBuilder.AuthStatus.class);
        } finally {
            releaseSecond.countDown();
            server.stop(0);
        }
