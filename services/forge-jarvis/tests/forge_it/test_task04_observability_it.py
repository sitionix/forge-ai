...
def test_it_obs_04_jarvis_observability_and_redaction(tmp_path):
    ...
    with TestClient(app) as client:
        ...
        chat = client.post("/api/v1/jarvis/chat", json={"message": "explain JarvisGateway"})

    for response in (status, actions, command, chat):
...
def test_it_obs_05_jarvis_correlation_id_is_sanitized_and_forwarded(tmp_path):
    ...
    with TestClient(app) as client:
        unsafe = client.post("/api/v1/jarvis/chat", json={"message": "explain"}, headers={"X-Correlation-Id": "unsafe value"})
        safe = client.post("/api/v1/jarvis/chat", json={"message": "explain"}, headers={"X-Correlation-Id": "corr-safe"})
...
def test_it_obs_06_jarvis_public_error_redaction(tmp_path):
    ...
    with TestClient(app) as client:
        command = client.post("/api/v1/jarvis/command", json={"text": "check ollama"})
        chat = client.post("/api/v1/jarvis/chat", json={"message": "explain"})
    ...
    for response in (command, chat, invalid_command):
