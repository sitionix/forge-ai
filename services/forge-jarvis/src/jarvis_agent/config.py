...
DEFAULT_FLOW_EXPLANATION_TRANSPORT_GRACE_SECONDS = 5
...
class FlowExplanationQuerySettings(BaseModel):
    request_timeout_seconds: float = Field(default=DEFAULT_FLOW_EXPLANATION_REQUEST_DEADLINE_SECONDS, gt=0)
...
class JarvisKnowledgeSettings(BaseModel):
    ...
    flow_explanation_transport_grace_seconds: float = Field(default=DEFAULT_FLOW_EXPLANATION_TRANSPORT_GRACE_SECONDS, gt=0)
...
class KnowledgeConfig(BaseModel):
    ...
    flow_explanation_request_timeout_seconds: float
...
class AppConfig(BaseModel):
    ...
    def from_forge_settings(cls, settings: ForgeSettings, environ: Optional[Mapping[str, str]] = None) -> "AppConfig":
        ...
        flow_deadline_seconds = settings.query.flow_explanation.request_timeout_seconds
        flow_transport_timeout_seconds = _flow_explanation_transport_timeout_seconds(
            flow_deadline_seconds,
            jarvis.knowledge.flow_explanation_transport_grace_seconds,
        )
        ...
        return cls(
            ...
            knowledge=KnowledgeConfig(
                ...
                flow_explanation_request_timeout_seconds=flow_transport_timeout_seconds,
            ),
                    ...
                    _expand(
                        str(
                            _field_value(
                                ...
                                default=DEFAULT_FLOW_EXPLANATION_REQUEST_DEADLINE_SECONDS,
                            )
                        ),
                        env,
                    )
        ...
        "services": {
            "jarvis": {
                ...
                "knowledge": {
                    ...
                    "flow_explanation_transport_grace_seconds": float(
                        _expand(
                            str(
                                _field_value(
                                    knowledge,
                                    "flow-explanation-transport-grace-seconds",
                                    "flow_explanation_transport_grace_seconds",
                                    default=DEFAULT_FLOW_EXPLANATION_TRANSPORT_GRACE_SECONDS,
                                )
                            ),
                            env,
                        )
...
def _apply_jarvis_env_overrides(raw: Dict[str, Any], env: Mapping[str, str]) -> None:
    ...
    if env.get("FORGE_FLOW_EXPLANATION_REQUEST_TIMEOUT_SECONDS"):
        raw["query"]["flow_explanation"]["request_timeout_seconds"] = float(env["FORGE_FLOW_EXPLANATION_REQUEST_TIMEOUT_SECONDS"])
    if env.get("JARVIS_KNOWLEDGE_FLOW_EXPLANATION_TRANSPORT_GRACE_SECONDS"):
        jarvis["knowledge"]["flow_explanation_transport_grace_seconds"] = float(
            env["JARVIS_KNOWLEDGE_FLOW_EXPLANATION_TRANSPORT_GRACE_SECONDS"]
        )


def _flow_explanation_transport_timeout_seconds(deadline_seconds: float, grace_seconds: float) -> float:
    timeout_seconds = deadline_seconds + grace_seconds
    if timeout_seconds <= deadline_seconds:
        raise ValueError("Jarvis Knowledge flow explanation transport timeout must exceed the Knowledge deadline")
