...
public class AgentProxyApiMapper {
  ...
  public CreateAgentProjectTaskCommand toCommand(final CreateAgentProjectTaskRequest request) {
    return new CreateAgentProjectTaskCommand(
        request.title(), request.input(), request.workflowId(), request.repositoryIds());
  }

  public ImportAgentProjectRepositoryCommand toCommand(
      final ImportAgentProjectRepositoryRequest request) {
    ...
    try {
      return new SaveAgentDefinitionCommand(
          ...
          new AgentOutputSchemaDocument(
              this.objectMapper.writeValueAsString(request.outputSchema())),
  ...
  public SaveAgentWorkflowCommand toCommand(final SaveAgentWorkflowRequest request) {
    return new SaveAgentWorkflowCommand(
        request.name(),
        request.nodes() == null ? null : request.nodes().stream().map(this::toDomain).toList(),
        request.connections() == null
            ? null
            : request.connections().stream().map(this::toDomain).toList(),
  ...
  public AgentProjectResponse toResponse(final AgentProject project) {
    return new AgentProjectResponse(
  ...
  private AgentProjectRepositoryGitStateResponse toResponse(
      final AgentProjectRepositoryGitState gitState) {
    if (gitState == null) {
      return null;
    }
    return new AgentProjectRepositoryGitStateResponse(
        gitState.branch(), gitState.workingTree(), gitState.pullAvailable());
  }
  ...
  private AgentRunPortResponse toResponse(final AgentRunPort port) {
    return new AgentRunPortResponse(
        port.sourcePortId(), port.sourceNodeId(), port.direction(), port.name(), port.order());
  }
  ...
  private AgentWorkflowRunExecutionEdgeResponse toResponse(
      final AgentWorkflowRunExecutionEdge edge) {
    return new AgentWorkflowRunExecutionEdgeResponse(
        edge.sourceNodeRunId(), edge.targetNodeRunId(), edge.sourceType());
  }

  private com.fasterxml.jackson.databind.JsonNode toJsonNode(
  ...
  private WorkflowConnection toDomain(final WorkflowConnectionRequest request) {
    ...
    return new WorkflowConnection(
        request.id(), request.sourceOutputPortId(), request.targetInputPortId());
  }
  ...
  private AgentNodeRunResponse toResponse(final AgentNodeRun nodeRun) {
    try {
      return new AgentNodeRunResponse(
          ...
          nodeRun.output() == null
              ? null
              : this.objectMapper.readTree(nodeRun.output().jsonValue()),
    ...
    try {
      return new AgentConnectionResolutionResponse(
          ...
          resolution.payload() == null
              ? null
              : this.objectMapper.readTree(resolution.payload().jsonValue()),
          resolution.consumedByNodeRunId(),
          resolution.createdAt(),
          resolution.targetRepositoryId());
    } catch (final JsonProcessingException exception) {
      throw new IllegalStateException(
          "Agent connection resolution payload is not valid JSON.", exception);
    }
  }
}
