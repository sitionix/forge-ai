...
public class McpConnectionsExceptionHandler {
    ...
    @ExceptionHandler(McpAgentClientException.class) public ResponseEntity<InfrastructureErrorResponse> upstream(McpAgentClientException error){
        return ResponseEntity.status(error.statusCode()).body(new InfrastructureErrorResponse(
                error.code(), error.upstreamMessage(), error.correlationId()));
    }
    @ExceptionHandler(Exception.class) public ResponseEntity<InfrastructureErrorResponse> failed(){return response(HttpStatus.BAD_GATEWAY,"UPSTREAM_INVALID_RESPONSE","Forge Agent service returned an invalid response.");}
