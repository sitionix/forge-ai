...
class ForgeAgentClientAdapterTest {
  ...
  @Mock private ForgeAgentHttpClient httpClient;
  @Mock private ForgeAgentClientMapper mapper;
  @Mock private ForgeAgentClientCallExecutor executor;
  @Mock private ForgeAgentLogStreamingHttpClient logStreamingHttpClient;
  ...
  void setUp() {
    ...
    this.adapter =
        new ForgeAgentClientAdapter(
            this.httpClient, this.mapper, this.executor, this.logStreamingHttpClient);
  }
  ...
  void importProjectRepositoryMapsRequestExecutesTypedClientCallAndMapsResponse() {
    final var command =
        new ImportAgentProjectRepositoryCommand("git@gitlab.com:company/service-a.git");
    final var request = new ImportProjectRepositoryRequest("git@gitlab.com:company/service-a.git");
    final var upstreamResponse =
        new ProjectRepositoryResponse(REPOSITORY_ID, PROJECT_ID, "service-a", false, null, CREATED);
    final var expected =
        new AgentProjectRepository(REPOSITORY_ID, PROJECT_ID, "service-a", false, null, CREATED);
    when(this.mapper.toRequest(command)).thenReturn(request);
  ...
  void cloneProjectRepositoryExecutesTypedClientCallAndMapsResponse() {
    final var upstreamResponse =
        new ProjectRepositoryResponse(REPOSITORY_ID, PROJECT_ID, "service-a", true, null, CREATED);
    final var expected =
        new AgentProjectRepository(REPOSITORY_ID, PROJECT_ID, "service-a", true, null, CREATED);
    when(this.httpClient.cloneProjectRepository(PROJECT_ID, REPOSITORY_ID))
  ...
  void pullProjectRepositoryExecutesTypedClientCallAndMapsResponse() {
    ...
    final var expected =
        new AgentProjectRepository(REPOSITORY_ID, PROJECT_ID, "service-a", true, null, CREATED);
    when(this.httpClient.pullProjectRepository(PROJECT_ID, REPOSITORY_ID))
        .thenReturn(upstreamResponse);
    when(this.mapper.toDomain(upstreamResponse)).thenReturn(expected);
  ...
  void refreshProjectRepositoryExecutesTypedClientCallAndMapsResponse() {
    ...
    final var expected =
        new AgentProjectRepository(REPOSITORY_ID, PROJECT_ID, "service-a", true, null, CREATED);
    when(this.httpClient.refreshProjectRepository(PROJECT_ID, REPOSITORY_ID))
        .thenReturn(upstreamResponse);
    when(this.mapper.toDomain(upstreamResponse)).thenReturn(expected);

    assertThat(this.adapter.refreshProjectRepository(PROJECT_ID, REPOSITORY_ID))
        .isEqualTo(expected);
  ...
  void createProjectTaskMapsRequestExecutesTypedClientCallAndMapsResponse() {
    ...
    final var command =
        new CreateAgentProjectTaskCommand(
            "Check calculation", "Count letters.", WORKFLOW_ID, repositoryIds);
    final var request =
  ...
  void listProjectTasksExecutesTypedClientCallAndMapsResponse() {
    final var summaryResponse =
        new ProjectTaskSummaryResponse(
            ...
            UPDATED);
    final var upstreamResponse =
        new ProjectTaskPageResponse(List.of(summaryResponse), 2, 10, 21, 3);
    final var expected =
        new AgentProjectTaskSummary(
            TASK_ID,
            PROJECT_ID,
            "Check calculation",
            WORKFLOW_ID,
  ...
  void getRuntimeExecutesTypedClientCallAndMapsResponse() {
    final var upstreamResponse =
        new AgentRuntimeResponse(
            List.of(
                new AgentRuntimeProviderResponse(
                    "codex", "Codex", AgentRuntimeProviderStatus.READY, "codex/1", List.of())));
    final var expected =
        new AgentRuntimeCatalog(
            List.of(
                new AgentRuntimeProvider(
                    "codex", "Codex", AgentRuntimeProviderStatus.READY, "codex/1", List.of())));
  ...
  void listProjectAgentsExecutesTypedClientCallAndMapsResponse() {
    final var upstreamResponse =
        new AgentDefinitionListResponse(AGENT_ID, PROJECT_ID, "Backend", null, CREATED, UPDATED);
    final var expected =
        new AgentDefinitionListItem(AGENT_ID, PROJECT_ID, "Backend", null, CREATED, UPDATED);
  ...
  void createAgentMapsRequestExecutesTypedClientCallAndMapsResponse() {
    final var command =
        new SaveAgentDefinitionCommand(
            "Backend", "Do work.", new AgentOutputSchemaDocument("{\"type\":\"object\"}"), null);
    final var request = new AgentDefinitionRequest("Backend", "Do work.", null, null);
    final var upstreamResponse =
        new AgentDefinitionResponse(
            AGENT_ID, PROJECT_ID, "Backend", "Do work.", null, null, CREATED, UPDATED);
    final var expected =
        new AgentDefinitionDetails(
            AGENT_ID,
            PROJECT_ID,
            "Backend",
            "Do work.",
  ...
  void listProjectWorkflowsExecutesTypedClientCallAndMapsResponse() {
    final var upstreamResponse =
        new AgentWorkflowResponse(
            WORKFLOW_ID, PROJECT_ID, "Full Testing", List.of(), List.of(), null, CREATED, UPDATED);
    final var expected =
        new AgentWorkflow(
            WORKFLOW_ID, PROJECT_ID, "Full Testing", List.of(), List.of(), null, CREATED, UPDATED);
  ...
  void updateWorkflowMapsRequestExecutesTypedClientCallAndMapsResponse() {
    ...
    final var request =
        new SaveAgentWorkflowRequest("Full Testing", List.of(), List.of(), null, null);
    final var upstreamResponse =
        new AgentWorkflowResponse(
            WORKFLOW_ID, PROJECT_ID, "Full Testing", List.of(), List.of(), null, CREATED, UPDATED);
    final var expected =
