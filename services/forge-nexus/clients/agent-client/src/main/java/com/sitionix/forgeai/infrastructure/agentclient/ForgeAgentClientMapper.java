...
public class ForgeAgentClientMapper {
    ...
    AgentExecutionContext toDomain(final AgentExecutionContextResponse response) {
        return new AgentExecutionContext(
                ...
                response.createdAt(),
                response.startedAt(),
                response.finishedAt(), response.contextResetAt(), response.resetAllowed(), response.resetReason(), response.contextIterationId(), response.contextGroupKey());
    }
    ...
    private AgentRunNode toDomain(final RunNodeResponse response) {
        return new AgentRunNode(
                ...
                response.scopeMode(),
                response.contextMode(), response.contextGroupKey(), response.nodeType(), response.resolvedWorkspaceRepositoryIds()
        );
    }
    ...
    private AgentNodeRunOutputDocument toOutputDocument(final JsonNode value) {
        if (value == null) {
            return null;
        }
        try {
    ...
    private Node toDomain(final NodeResponse response) {
        ...
        return new Node(
                ...
                response.scopeMode(),
                response.contextMode(), response.contextGroupKey(), response.nodeType(), response.includeTaskRepositories(), response.workspaceRepositoryIds()
        );
    }
