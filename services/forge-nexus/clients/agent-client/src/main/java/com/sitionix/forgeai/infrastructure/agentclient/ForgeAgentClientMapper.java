...
public class ForgeAgentClientMapper {
  ...
  CreateProjectTaskRequest toRequest(final CreateAgentProjectTaskCommand command) {
    return new CreateProjectTaskRequest(
        command.title(), command.input(), command.workflowId(), command.repositoryIds());
  }
  ...
  AgentProject toDomain(final AgentProjectResponse response) {
    return new AgentProject(
        response.id(), response.name(), response.createdAt(), response.updatedAt());
  }
  ...
  private AgentProjectRepositoryGitState toDomain(
      final ProjectRepositoryGitStateResponse response) {
    if (response == null) {
      return null;
    }
    return new AgentProjectRepositoryGitState(
        response.branch(), response.workingTree(), response.pullAvailable());
  }
  ...
  AgentProjectTaskPage toDomain(final ProjectTaskPageResponse response) {
    return new AgentProjectTaskPage(
        response.items() == null ? null : response.items().stream().map(this::toDomain).toList(),
        response.page(),
        response.size(),
        response.totalItems(),
        response.totalPages());
  ...
  AgentProjectTask toDomain(final ProjectTaskResponse response) {
    return new AgentProjectTask(
        ...
        response.runs() == null ? null : response.runs().stream().map(this::toDomain).toList(),
  ...
  AgentDefinitionDetails toDomain(final AgentDefinitionResponse response) {
    try {
      return new AgentDefinitionDetails(
          ...
          response.outputSchema() == null
              ? null
              : new AgentOutputSchemaDocument(
                  this.objectMapper.writeValueAsString(response.outputSchema())),
          this.toDomain(response.model()),
          response.createdAt(),
          response.updatedAt());
    } catch (final JsonProcessingException exception) {
      throw new IllegalArgumentException(
          "Forge Agent response output schema was invalid.", exception);
    }
  }
  ...
  SaveAgentWorkflowRequest toRequest(final SaveAgentWorkflowCommand command) {
    return new SaveAgentWorkflowRequest(
        command.name(),
        command.nodes() == null ? null : command.nodes().stream().map(this::toRequest).toList(),
        command.connections() == null
            ? null
            : command.connections().stream().map(this::toRequest).toList(),
  ...
  AgentWorkflow toDomain(final AgentWorkflowResponse response) {
    return new AgentWorkflow(
        ...
        response.projectId(),
        response.name(),
        response.nodes() == null ? null : response.nodes().stream().map(this::toDomain).toList(),
        response.connections() == null
            ? null
            : response.connections().stream().map(this::toDomain).toList(),
  ...
  AgentWorkflowRun toDomain(final WorkflowRunResponse response) {
    return new AgentWorkflowRun(
        ...
        response.input(),
        response.status(),
        response.nodeRuns() == null
            ? null
            : response.nodeRuns().stream().map(this::toDomain).toList(),
        response.connectionResolutions() == null
            ? null
            : response.connectionResolutions().stream().map(this::toDomain).toList(),
        response.executionEdges() == null
            ? null
            : response.executionEdges().stream().map(this::toDomain).toList(),
  ...
  private AgentWorkflowRunGraph toDomain(final WorkflowRunGraphResponse response) {
    ...
    return new AgentWorkflowRunGraph(
        ...
        response.taskOutputPortId(),
        response.nodes() == null ? null : response.nodes().stream().map(this::toDomain).toList(),
        response.ports() == null ? null : response.ports().stream().map(this::toDomain).toList(),
        response.connections() == null
            ? null
            : response.connections().stream().map(this::toDomain).toList());
  }
  ...
  private AgentRunConnection toDomain(final RunConnectionResponse response) {
    return new AgentRunConnection(
        response.sourceConnectionId(), response.sourceOutputPortId(), response.targetInputPortId());
  }
  ...
  private AgentWorkflowRunExecutionEdge toDomain(final WorkflowRunExecutionEdgeResponse response) {
    return new AgentWorkflowRunExecutionEdge(
        response.sourceNodeRunId(), response.targetNodeRunId(), response.sourceType());
  }

  private AgentNodeRunOutputDocument toOutputDocument(
      final com.fasterxml.jackson.databind.JsonNode value) {
    if (value == null) {
      return null;
    }
    try {
  ...
  AgentRuntimeCatalog toDomain(final AgentRuntimeResponse response) {
    return new AgentRuntimeCatalog(
        response.providers() == null
            ? null
            : response.providers().stream().map(this::toDomain).toList());
  }
  ...
  private AgentRuntimeModel toDomain(final AgentRuntimeModelResponse response) {
    return new AgentRuntimeModel(
        ...
        response.efforts() == null
            ? null
            : response.efforts().stream().map(this::toDomain).toList());
  }
  ...
  private NodeRequest toRequest(final Node node) {
    return new NodeRequest(
        ...
        node.position() == null
            ? null
            : new NodePositionRequest(node.position().x(), node.position().y()),
        node.scopeMode());
  }
  ...
  private Node toDomain(final NodeResponse response) {
    ...
    return new Node(
        ...
        response.outputs() == null
            ? null
            : response.outputs().stream().map(this::toDomain).toList(),
        position == null ? null : new NodePosition(position.x(), position.y()),
        response.scopeMode());
  }

  private com.sitionix.forgeai.infrastructure.agentclient.dto.WorkflowConnectionRequest toRequest(
    ...
    return new com.sitionix.forgeai.infrastructure.agentclient.dto.WorkflowConnectionRequest(
        connection.id(), connection.sourceOutputPortId(), connection.targetInputPortId());
  }

  private WorkflowConnection toDomain(
      final com.sitionix.forgeai.infrastructure.agentclient.dto.WorkflowConnectionResponse
          response) {
    return new WorkflowConnection(
        response.id(), response.sourceOutputPortId(), response.targetInputPortId());
  }

  private com.sitionix.forgeai.infrastructure.agentclient.dto.NodePortRequest toRequest(
      final NodePort port) {
    return new com.sitionix.forgeai.infrastructure.agentclient.dto.NodePortRequest(
        port.id(), port.name(), port.description(), port.order());
  }

  private NodePort toDomain(
      final com.sitionix.forgeai.infrastructure.agentclient.dto.NodePortResponse response) {
  ...
  private AgentNodeRun toDomain(final NodeRunResponse response) {
    try {
      return new AgentNodeRun(
          ...
          response.agentOutputSchema() == null
              ? null
              : new AgentOutputSchemaDocument(
                  this.objectMapper.writeValueAsString(response.agentOutputSchema())),
          response.inputMode(),
          response.position() == null
              ? null
              : new NodePosition(response.position().x(), response.position().y()),
          ...
          response.output() == null
              ? null
              : new AgentNodeRunOutputDocument(
                  this.objectMapper.writeValueAsString(response.output())),
  ...
  private AgentConnectionResolution toDomain(final ConnectionResolutionResponse response) {
    try {
      return new AgentConnectionResolution(
          ...
          response.resolutionType() == null
              ? null
              : ConnectionResolutionType.valueOf(response.resolutionType().name()),
          response.payload() == null
              ? null
              : new AgentNodeRunOutputDocument(
                  this.objectMapper.writeValueAsString(response.payload())),
          response.consumedByNodeRunId(),
          response.createdAt(),
          response.targetRepositoryId());
    } catch (final JsonProcessingException exception) {
      throw new IllegalArgumentException(
          "Forge Agent connection resolution payload was invalid.", exception);
    }
  }
}
