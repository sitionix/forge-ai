...
public class LaneStepPromptBuilder {
    ...
    private static final String RESULT_CONTRACT = """
            ...
            - evidence must contain only active-step validation facts
            - do not copy task payloads, previous step results, full prompts, or full responses into evidence
    ...
    public String buildStepPrompt(final ReadyToStartLane lane,
        ...
        final StringBuilder prompt = new StringBuilder()
                ...
                .append("- validator: ").append(this.renderOptional(step.getValidator())).append('\n')
                .append('\n')
                ...
                .append("- scope: ").append(lane.getScope()).append('\n')
                .append('\n')
                .append("Step boundary:\n")
                .append("- Previous steps are already validated and persisted by the orchestrator.\n")
                .append("- Do not repeat previous step work or re-check previous step evidence.\n")
                .append("- Execute only the active step responsibility from the active instructions below.\n")
                .append("- If a previous step was wrong, that is a validator problem; do not compensate in this step.\n")
                .append("- Return minimal evidence for the active step only.\n")
                .append('\n');
    ...
    private String renderOptional(final String value) {
