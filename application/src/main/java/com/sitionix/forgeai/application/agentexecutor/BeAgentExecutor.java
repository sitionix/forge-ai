...
public class BeAgentExecutor extends SupervisedTaskDrivenAgentExecutor implements ExecuteAgent<ImplementBePayload> {
    ...
    public void completeLane(final ReadyToStartLane lane, final Map<String, Object> completionPayload) {
        ...
        this.completeAgentTasks.complete(lane.getLaneId(), List.of(
                this.laneCompletionSupport.ticket(lane, this.laneCompletionSupport.convert(completionPayload, TestUnitPayload.class), lane.getScope(), Agent.TEST_UNIT),
                this.laneCompletionSupport.ticket(lane, this.laneCompletionSupport.convert(completionPayload, TestItPayload.class), lane.getScope(), Agent.TEST_IT)
        ));
    }
}
