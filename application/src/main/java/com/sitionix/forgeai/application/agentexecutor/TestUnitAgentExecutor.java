...
public class TestUnitAgentExecutor extends SupervisedTaskDrivenAgentExecutor implements ExecuteAgent<TestUnitPayload> {
    ...
    public void completeLane(final ReadyToStartLane lane, final Map<String, Object> completionPayload) {
        ...
        this.completeAgentTasks.complete(lane.getLaneId(), List.of(
                this.laneCompletionSupport.ticket(
                        lane,
                        this.laneCompletionSupport.convert(completionPayload, ReviewerPayload.class),
                        this.laneCompletionSupport.globalScope(),
                        Agent.REVIEWER
                )
        ));
    }
