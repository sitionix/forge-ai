...
PR #147 was merged with authorized admin merge at `58f2854f`; the user authorized Stage 7.
Stage 7 PR #149 was merged at `87c6da2b`; the user authorized Stage 8.
Each stage remains a separate review unit; Stage 9 remains unauthorized.
...
| 6 | Agent local management API and typed Nexus proxy | Production HTTP auth/Origin/CSRF, error/status preservation, redaction; ForgeIT typed fixtures, zero upstream calls on rejection | Merged PR #147 including dedicated 120s Nexus lifecycle timeout; stage6-management.md and evidence.md |
| 7 | Global Remote Access Console | Connect/Give Access; role-separated lists; no ACTIVE=Online; REVOKING visible; stale/double-submit/polling/secret tests and existing UI regression | Merged PR #149; stage7-console.md and evidence.md |
| 8 | Local `forge-remote exec` helper | Actual remote read/edit/test, literal argv, cancellation, no local fallback or secret output; local Codex tools remain local | Implemented for external review; stage8-local-helper.md and evidence.md |
...
REMOTE_ACCESS_CODEX_LIVE_PASS require their respective real evidence; Stage 7 records a real-browser UI fixture pass with stubbed management HTTP;
full Stage 9 runtime/two-machine/live-Codex acceptance remains NOT_RUN.
