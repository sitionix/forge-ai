...
**Architecture:** Ordinary Forge serves a local-only, authenticated cold-state control path. A narrow root-owned bootstrap prepares the dedicated Remote Access runtime only on Enable/Connect. The existing one-way SSH admission machinery is used twice in one durable pairing attempt; the browser calls the pair ready only after both grants are ACTIVE.
...
**Interfaces:**
- Consumes: local operator cookie, exact loopback Origin, CSRF token, `prepare_startup.py` fixed setup command.
- Produces: typed cold-state `GET /control` and `POST /control/enable`, plus a root bootstrap socket accepting only a fixed `PREPARE` operation from the operator identity.

- [ ] Write a failing startup regression: invoke the `just start` shell path with mocked service commands; assert no call to `stage-remote-access.sh`, `prepare_startup.py`, apt, managed sshd start, or dedicated service start. Assert ordinary Agent/Nexus health waits remain.
...
- [ ] Write a failing Nexus HTTP test: on the ordinary loopback Nexus with Remote Access disabled, the operator can load control state and call Enable only with valid cookie, Origin, and CSRF; unauthenticated requests cannot invoke bootstrap.
- [ ] Run the focused Python and Nexus tests; record the expected failures.
- [ ] Implement a root-owned socket-activated bootstrap, with `SO_PEERCRED` authorization and fixed command dispatch. Install only this small helper/socket at ordinary startup. Move package staging, OpenSSH installation, DB preparation, rootfs preparation, and dedicated service launch behind `PREPARE`. Keep an explicit in-progress result and fail closed on partial setup.
- [ ] Make the ordinary Nexus serve the authenticated cold control path. After successful setup, hand off to the dedicated typed Remote Access client. No raw JSON proxy or generic shell executor.
...
- [ ] Add a typed reciprocal message on the authenticated first SSH channel: connector endpoint/host public key; inviter's newly generated reverse session public key; connector's bound grant acknowledgement. Pin the connector host key before inviter opens reverse SSH. Keep private keys local to their owners.
...
- [ ] Run a privileged isolated fixture with two Forge instances: cold startup has no managed SSH listener; one token forms two authenticated grants; each direction runs a command; revoking either inbound grant stops its own commands and does not invent peer cleanup.
