...
`just start` prepares the reviewed Remote Access package in a root-owned, non-writable location and runs the existing installers from there. It creates or verifies the dedicated users, managed SSH configuration, host key, runtime directories, workload and invitation supervisors, protected operator/service secret files, Stage 8 local-exec socket/helper, and systemd units. It creates a separate Remote Access database so the dedicated Agent cannot race the ordinary Agent's workflow recovery. It prepares a root-owned, immutable, credential-free workload root filesystem with the supported command/test tools; missing toolchain or image preparation is a visible `NOT_READY` failure, not a late command surprise. It starts the dedicated supervisors, Agent, and Nexus in dependency order and checks readiness. Ordinary Forge services remain separate. A repeated start preserves host identity, credentials, session records, and the persisted switch. It does not silently re-enable a disabled installation.
...
The dedicated Agent uses a root-owned packaged JAR, an explicit loopback bind, protected DB configuration, and the established Remote Access channel and management flags. It must not execute a JAR from an operator- or workload-writable checkout. The management Nexus uses an explicit loopback bind, protected service/operator secrets, the existing operator Origin/CSRF/session boundary, and the dedicated read timeout. Neither service shares the ordinary wildcard HTTP listener. `just start` reports the local management URL and protected operator credential location, never the credential value in logs. Operator sign-in remains required before Enable, Disable, Give Access, or Connect.

The existing Stage 5 `prepare-workspace` operation is session-specific: `just start` cannot call it before session IDs exist. On GRANTOR pairing, a narrow privileged operation prepares exactly that session's isolated user, directory, and manifest from the trusted rootfs before reporting the session `ACTIVE`. Failure keeps pairing non-active and leaves safe diagnostics; no generic root-command API is exposed. Existing physical workspace files are not removed merely by Disable, but their session authorization and processes are removed. A new session must not inherit a prior session's workspace or identity.
...
On load, the page obtains the operator session and switch/readiness status. Its first screen gives one clear next action: **Enable Remote Access** when ready and disabled, or **Connect** / **Give Access** when enabled. It explains the selected local SSH endpoint in human terms. Give Access and Connect are unavailable until Enable succeeds, with an inline reason rather than an unexplained disabled button. When enabled, existing invitation/session controls work unchanged and a visually secondary **Disable Remote Access** action appears. Disable requests confirmation that all sessions will be revoked and running remote commands terminated. The page shows progress, how many sessions remain unresolved, and the exact safe pending reason while `DISABLING`; it offers retry and keeps existing session cards visible. A failed HTTP request never becomes a success state. When bootstrap is incomplete, the page shows the concrete `NOT_READY` diagnostic and the next action available to the operator instead of the current generic 404 message.

Use a dedicated status region with accessible text and `aria-live` updates, keyboard-reachable controls, focus restoration after the Disable confirmation, and responsive layout. Disable double submission and protect against stale responses after navigation or a newer action. Do not expose raw exception text, secret paths, or tokens in diagnostics. Preserve the existing one-time token and pairing UX.
...
- Console/Nexus: readiness, enable, disable, pending and error rendering; meaningful disabled-control explanations; confirmation and focus behavior; keyboard and responsive behavior; stale-response and double-click handling; typed forwarding; no secret persistence; ordinary Forge pages and existing Remote Access invitation/connect tests stay green.
